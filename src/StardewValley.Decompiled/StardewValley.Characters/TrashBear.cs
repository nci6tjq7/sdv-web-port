...
public class TrashBear : NPC
{
	...
	public void updateItemWanted()
	{
		...
		int num2 = Utility.CreateRandomSeed(777111.0, num);
		itemWantedIndex = Utility.getRandomPureSeasonalItem(Game1.season, num2);
		if (num <= 1)
		{
			return;
		}
		int num3 = Utility.CreateRandom(Game1.uniqueIDForThisGame, num2).Next(CraftingRecipe.cookingRecipes.Count);
		int num4 = 0;
		foreach (string value2 in CraftingRecipe.cookingRecipes.Values)
		{
			if (num4 == num3)
			{
				string value = ArgUtility.Get(value2.Split('/'), 2);
				value = ArgUtility.SplitBySpaceAndGet(value, 0);
				itemWantedIndex = value;
				break;
			}
			num4++;
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (showWantBubbleTimer > 0)
		{
			float num = 2f * (float)Math.Round(Math.Sin(Game1.currentGameTime.TotalGameTime.TotalMilliseconds / 250.0), 2);
			Point tilePoint = base.TilePoint;
			float num2 = (float)((tilePoint.Y + 1) * 64) / 10000f;
			num -= 40f;
			b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tilePoint.X * 64 + 32, (float)(tilePoint.Y * 64 - 96 - 48) + num)), new Rectangle(141, 465, 20, 24), Color.White * 0.75f, 0f, Vector2.Zero, 4f, SpriteEffects.None, num2 + 1E-06f);
			ParsedItemData dataOrErrorItem = ItemRegistry.GetDataOrErrorItem("(O)" + itemWantedIndex);
			Texture2D texture = dataOrErrorItem.GetTexture();
			Rectangle sourceRect = dataOrErrorItem.GetSourceRect();
			b.Draw(texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(tilePoint.X * 64 + 64 + 8, (float)(tilePoint.Y * 64 - 64 - 32 - 8) + num)), sourceRect, Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, num2 + 1E-05f);
		}
	}
}
