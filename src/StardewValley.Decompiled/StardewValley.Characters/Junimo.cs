...
public class Junimo : NPC
{
	...
	private void finalCutscene()
	{
		...
		Game1.UpdateViewPort(overrideFreeze: true, standingPixel);
	...
	public void bringBundleBackToHut(Color bundleColor, GameLocation location)
	{
		...
		int num = 0;
		while (location.isCollidingPosition(GetBoundingBox(), Game1.viewport, this) && num < 5)
		{
			base.Position = Utility.getRandomAdjacentOpenTile(Game1.player.Tile, location) * 64f;
			num++;
		}
	...
	public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
	{
		if (textAboveHeadTimer > 0 && textAboveHead != null)
		{
			...
			Vector2 vector = Game1.GlobalToLocal(new Vector2(standingPixel.X, (float)standingPixel.Y - 128f + (float)yJumpOffset));
			if (textAboveHeadStyle == 0)
			{
				vector += new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2));
			}
			SpriteText.drawStringWithScrollCenteredAt(b, textAboveHead, (int)vector.X, (int)vector.Y, "", textAboveHeadAlpha, textAboveHeadColor, 1, (float)(base.TilePoint.Y * 64) / 10000f + 0.001f + (float)base.TilePoint.X / 10000f, !sayingGoodbye.Value);
		}
	}
	...
	public override void update(GameTime time, GameLocation location)
	{
		...
		else
		{
			...
			if (!base.IsInvisible && farmerCloseCheckTimer <= 0 && controller == null && alpha.Value >= 1f && !stayPut.Value && Game1.IsMasterGame)
			{
				...
				{
					Farmer farmer = Utility.isThereAFarmerWithinDistance(base.Tile, 5, base.currentLocation);
					if (farmer != null)
					{
						if (friendly.Value && Vector2.Distance(base.Position, farmer.Position) > (float)(base.speed * 4))
						{
							...
							setMoving(Utility.getVelocityTowardPlayer(new Point((int)base.Position.X, (int)base.Position.Y), base.speed, farmer));
						}
						else if (!friendly.Value)
						{
							...
							setMoving(awayFromPlayerTrajectory * base.speed);
						}
			...
			if (!base.IsInvisible && controller == null)
			{
				...
				if (!motion.Equals(Vector2.Zero) && flag && Game1.random.NextDouble() < 0.005)
				{
