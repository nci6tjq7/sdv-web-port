...
public class GalaxyNetServer : HookableServer
{
	...
	public override string getUserName(long farmerId)
	{
		...
		ulong num = peers[farmerId];
		if (displayNames.TryGetValue(num, out var value))
		{
			return value;
		}
		GalaxyID userID = new GalaxyID(num);
	...
	public override float getPingToClient(long farmerId)
	{
		...
		GalaxyID peer = new GalaxyID(peers[farmerId]);
		return server.GetPingWith(peer);
	}
	...
	private void onProfileDataReady(GalaxyID userID)
	{
		if (!(userID == host) && !displayNames.ContainsKey(userID.ToUint64()))
		{
			...
			try
			{
				value = GalaxyInstance.User().GetUserData("StardewDisplayName", userID);
			}
			catch (Exception)
			{
			}
			...
			{
				displayNames[userID.ToUint64()] = value;
				Game1.log.Verbose($"{userID} ({value}) connected");
			}
			else
			{
	...
	public override void receiveMessages()
	{
		...
		foreach (GalaxyID connection in server.Connections)
		{
			if (server.GetPingWith(connection) > 30000)
			{
	...
	public override void kick(long disconnectee)
	{
		...
		{
			GalaxyID galaxyID = new GalaxyID(peers[disconnectee]);
			server.Kick(galaxyID);
			sendMessage(galaxyID, new OutgoingMessage(23, Game1.player));
		}
	}
