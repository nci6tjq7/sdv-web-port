...
public class AbigailGame : IMinigame
{
	...
	public virtual void SetupBinds()
	{
		...
		if (boundKey != 0 && boundKey != Keys.Up && boundKey != Keys.Down && boundKey != Keys.Left && boundKey != Keys.Right)
		{
	...
	public bool tick(GameTime time)
	{
		...
		if (motionPause <= 0)
		{
			...
			if (shopping)
			{
				...
				{
					for (int num7 = storeItems.Count - 1; num7 >= 0; num7--)
					{
						KeyValuePair<Rectangle, int> keyValuePair = storeItems.ElementAt(num7);
			...
			{
				Vector2 item2 = new Vector2((powerup.position.X + TileSize / 2) / TileSize, (powerup.position.Y + TileSize / 2) / TileSize);
				Vector2 item3 = new Vector2(powerup.position.X / TileSize, powerup.position.Y / TileSize);
				Vector2 item4 = new Vector2((powerup.position.X + TileSize) / TileSize, powerup.position.Y / TileSize);
				Vector2 item5 = new Vector2(powerup.position.X / TileSize, powerup.position.Y / TileSize);
				Vector2 item6 = new Vector2(powerup.position.X / TileSize, (powerup.position.Y + 64) / TileSize);
				if (_borderTiles.Contains(item2) || _borderTiles.Contains(item3) || _borderTiles.Contains(item4) || _borderTiles.Contains(item5) || _borderTiles.Contains(item6))
				{
					Point point = default(Point);
					if (Math.Abs(item2.X - 8f) > Math.Abs(item2.Y - 8f))
					{
						point.X = Math.Sign(item2.X - 8f);
					}
					else
					{
						point.Y = Math.Sign(item2.Y - 8f);
					}
			...
			else if (deathTimer <= 0f && !waitingForPlayerToMoveDownAMap && !shopping && !shootoutLevel)
			{
				if (waveTimer > 0)
				{
					...
					if (playingWithAbigail && num8 > 40000 && waveTimer <= 40000)
					{
					...
					foreach (Vector2 monsterChance in monsterChances)
					{
						...
						{
							int num10 = 1;
							while (Game1.random.NextDouble() < (double)monsterChance.Y && num10 < 15)
							{
								num10++;
							}
							spawnQueue[(whichWave == 11) ? (Game1.random.Next(1, 3) * 2 - 1) : Game1.random.Next(4)].Add(new Point(num9, num10));
						}
						num9++;
					}
					if (!hasGopherAppeared && monsters.Count > 6 && Game1.random.NextDouble() < 0.0004 && waveTimer > 7000 && waveTimer < 50000)
					{
						...
						int num11 = 0;
						while ((isCollidingWithMap(gopherBox) || isCollidingWithMonster(gopherBox, null) || Math.Abs((float)gopherBox.X - playerPosition.X) < (float)(TileSize * 6) || Math.Abs((float)gopherBox.Y - playerPosition.Y) < (float)(TileSize * 6) || Math.Abs(gopherBox.X - 8 * TileSize) < TileSize * 4 || Math.Abs(gopherBox.Y - 8 * TileSize) < TileSize * 4) && num11 < 10)
						{
				...
				for (int n = 0; n < 4; n++)
				{
					...
					if (spawnQueue[n][0].X == 1 || spawnQueue[n][0].X == 4)
					{
						...
						int num12 = 0;
						while (isCollidingWithMonster(new Rectangle((int)vector2.X * TileSize, (int)vector2.Y * TileSize, TileSize, TileSize), null) && num12 < 10)
						{
						...
						if (num12 < 10)
						{
							CowboyMonster cowboyMonster = new CowboyMonster(spawnQueue[n][0].X, new Point((int)vector2.X * TileSize, (int)vector2.Y * TileSize));
							if (whichRound > 0)
							{
					...
					case 0:
					{
						...
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(num16 * 16 * 3, 0, 48, 48), null))
							{
								CowboyMonster cowboyMonster5 = new CowboyMonster(spawnQueue[n][0].X, new Point(num16 * TileSize, 0));
								if (whichRound > 0)
								{
					...
					case 1:
					{
						for (int num14 = 7; num14 < 10; num14++)
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(720, num14 * TileSize, 48, 48), null))
							{
								CowboyMonster cowboyMonster3 = new CowboyMonster(spawnQueue[n][0].X, new Point(15 * TileSize, num14 * TileSize));
								if (whichRound > 0)
								{
					...
					case 2:
					{
						...
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(num15 * 16 * 3, 15 * TileSize, 48, 48), null))
							{
								CowboyMonster cowboyMonster4 = new CowboyMonster(spawnQueue[n][0].X, new Point(num15 * TileSize, 15 * TileSize));
								if (whichRound > 0)
								{
					...
					case 3:
					{
						for (int num13 = 7; num13 < 10; num13++)
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(0, num13 * TileSize, 48, 48), null))
							{
								CowboyMonster cowboyMonster2 = new CowboyMonster(spawnQueue[n][0].X, new Point(0, num13 * TileSize));
								if (whichRound > 0)
								{
				...
				if (waveTimer <= 0 && monsters.Count == 0 && isSpawnQueueEmpty())
				{
					...
					if (whichRound > 0)
					{
						for (int num17 = 0; num17 < monsterChances.Count; num17++)
						{
							_ = monsterChances[num17];
							monsterChances[num17] *= 1.1f;
						}
					}
			...
			for (int num18 = monsters.Count - 1; num18 >= 0; num18--)
			{
				monsters[num18].move(playerPosition, time);
				if (num18 < monsters.Count && monsters[num18].position.Intersects(playerBoundingBox) && playerInvincibleTimer <= 0)
				{
					if (zombieModeTimer <= 0)
					{
						playerDie();
					...
					if (monsters[num18].type != -2)
					{
						addGuts(monsters[num18].position.Location, monsters[num18].type);
						monsters.RemoveAt(num18);
						Game1.playSound("Cowboy_monsterDie");
					}
				}
				if (playingWithAbigail && num18 < monsters.Count && monsters[num18].position.Intersects(player2BoundingBox) && player2invincibletimer <= 0)
				{
					Game1.playSound("Cowboy_monsterDie");
