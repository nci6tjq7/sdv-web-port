...
public class MineCart : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		float num9 = (screenLeftBound - num3) / (float)tileSize;
		lakeSpeedAccumulator += (float)num2 * (num9 / 4f) % 96f;
		backBGPosition += (float)num2 * (num9 / 5f);
		backBGPosition = (backBGPosition + 9600f) % 96f;
		midBGPosition += (float)num2 * (num9 / 4f);
		midBGPosition = (midBGPosition + 9600f) % 96f;
		waterFallPosition += (float)num2 * (num9 * 6f / 5f);
		if (waterFallPosition > (float)(screenWidth * 3 / 2))
		{
			waterFallPosition %= screenWidth * 3 / 2;
			waterfallWidth = Game1.random.Next(6);
		}
		for (int num10 = sparkShower.Count - 1; num10 >= 0; num10--)
		{
			sparkShower[num10].dy += 0.105f * (num / 0.0166666f);
			sparkShower[num10].x += sparkShower[num10].dx * (num / 0.0166666f);
			sparkShower[num10].y += sparkShower[num10].dy * (num / 0.0166666f);
			sparkShower[num10].c.B = (byte)(0.0 + Math.Max(0.0, Math.Sin(totalTimeMS / (Math.PI * 20.0 / (double)sparkShower[num10].dx)) * 255.0));
			if (reachedFinish)
			{
				sparkShower[num10].c.R = (byte)(0.0 + Math.Max(0.0, Math.Sin((totalTimeMS + 50.0) / (Math.PI * 20.0 / (double)sparkShower[num10].dx)) * 255.0));
				sparkShower[num10].c.G = (byte)(0.0 + Math.Max(0.0, Math.Sin((totalTimeMS + 100.0) / (Math.PI * 20.0 / (double)sparkShower[num10].dx)) * 255.0));
				if (sparkShower[num10].c.R == 0)
				{
					sparkShower[num10].c.R = byte.MaxValue;
				}
				if (sparkShower[num10].c.G == 0)
				{
					sparkShower[num10].c.G = byte.MaxValue;
				}
			}
			if (sparkShower[num10].y > (float)screenHeight)
			{
				sparkShower.RemoveAt(num10);
			}
		}
		return false;
	}
