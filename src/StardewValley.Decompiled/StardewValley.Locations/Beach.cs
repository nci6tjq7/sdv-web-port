...
public class Beach : GameLocation
{
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		bool flag = true;
		for (float num = position.Y / 64f; num < (float)map.RequireLayer("Back").LayerHeight; num += 1f)
		{
			if (!isWaterTile((int)position.X / 64, (int)num) || !isWaterTile((int)position.X / 64 - 1, (int)num) || !isWaterTile((int)position.X / 64 + 1, (int)num))
			{
				flag = false;
				break;
			}
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		while (Game1.random.NextDouble() < (double)num)
		{
			...
			{
				dropObject(ItemRegistry.Create<Object>(itemId), vector * 64f, Game1.viewport, initialPlacement: true);
			}
			num /= 2f;
		}
		...
		while (Game1.random.NextDouble() < (double)num)
		{
			if (Game1.random.NextDouble() < 0.1)
			{
				...
				if (CanItemBePlacedHere(vector2))
				{
					dropObject(ItemRegistry.Create<Object>("(O)152"), vector2 * 64f, Game1.viewport, initialPlacement: true);
				}
			}
			num /= 2f;
		}
		if (IsSummerHere() && Game1.dayOfMonth >= 12 && Game1.dayOfMonth <= 14)
		{
			...
			while (Game1.random.NextDouble() < (double)num)
			{
				string itemId2 = ((Game1.random.NextDouble() < 0.2) ? "(O)397" : "(O)393");
				Vector2 randomTile = getRandomTile();
				randomTile.Y /= 2f;
				string text = doesTileHaveProperty((int)randomTile.X, (int)randomTile.Y, "Type", "Back");
				if (CanItemBePlacedHere(randomTile) && (text == null || !text.Equals("Wood")))
				{
					dropObject(ItemRegistry.Create<Object>(itemId2), randomTile * 64f, Game1.viewport, initialPlacement: true);
				}
				num /= 1.1f;
			}
		}
	...
	public override void performTenMinuteUpdate(int timeOfDay)
	{
		...
		NPC characterFromName = getCharacterFromName("winter_derby_contestent" + random.Next(10));
		...
		if (random.NextBool(0.25))
		{
			...
			if (num == 0 || num == 6)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite(151, 1500f, 1, 1, characterFromName.Position, flicker: false, flipped: false, verticalFlipped: false, 0f)
				{
	...
	public override void MakeMapModifications(bool force = false)
	{
		...
		if (Game1.IsWinter && Game1.dayOfMonth >= 12 && Game1.dayOfMonth <= 13)
		{
			...
			{
				string addedMapOverrideTilesheetId = GameLocation.GetAddedMapOverrideTilesheetId("Beach_SquidFest", "16");
				setMapTile(13, 6, 51, "Front", addedMapOverrideTilesheetId);
				setMapTile(13, 5, 43, "AlwaysFront", addedMapOverrideTilesheetId);
			}
	...
	protected override void resetLocalState()
	{
		...
		foreach (Vector2 item in Utility.getPositionsInClusterAroundThisTile(new Vector2(Game1.random.Next(map.DisplayWidth / 64), Game1.random.Next(12, map.DisplayHeight / 64)), number))
		{
			if (!isTileOnMap(item) || (!CanItemBePlacedHere(item) && !isWaterTile((int)item.X, (int)item.Y)) || (!(item.X < 23f) && !(item.X > 46f)))
			{
				continue;
			}
			int startingState = 3;
			if (isWaterTile((int)item.X, (int)item.Y) && doesTileHaveProperty((int)item.X, (int)item.Y, "Passable", "Buildings") == null)
			{
				startingState = 2;
