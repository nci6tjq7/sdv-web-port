...
public class IslandWestCave1 : IslandLocation
{
	...
	public override bool performAction(string[] action, Farmer who, Location tileLocation)
	{
		if (who.IsLocalPlayer)
		{
			...
			{
				if (!ArgUtility.TryGetInt(action, 1, out var value, out var error, "int crystalId"))
				{
	...
	public virtual void UpdateActivationTiles()
	{
		if (map != null && Game1.gameMode != 6 && Game1.currentLocation == this)
		{
			int index = ((isActivated.Value || completed.Value) ? 33 : 31);
			setMapTile(6, 1, index, "Buildings", "untitled tile sheet");
		}
	}
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		int num = currentCrystalSequence[currentPlaybackCrystalSequenceIndex];
		if (num < crystals.Count)
		{
			crystals[num].activate();
		}
		currentPlaybackCrystalSequenceIndex++;
		int num2 = currentDifficulty.Value;
		...
		betweenNotesTimer = 1500f / (float)num2;
		if (currentDifficulty.Value > ((timesFailed.Value < 8) ? 7 : 6))
		{
	...
	public override void TransferDataFromSavedLocation(GameLocation l)
	{
		...
		if (l is IslandWestCave1 islandWestCave)
		{
			completed.Value = islandWestCave.completed.Value;
		}
	}
