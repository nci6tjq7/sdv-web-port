...
public class BoatTunnel : GameLocation
{
	...
	public override bool RunLocationSpecificEventCommand(Event current_event, string command_string, bool first_run, params string[] args)
	{
		...
		case "path_player":
		{
			int @int = ArgUtility.GetInt(args, 0);
			int int2 = ArgUtility.GetInt(args, 1);
			int int3 = ArgUtility.GetInt(args, 2, 2);
			if (first_run)
			{
				_playerPathing = true;
				Game1.player.controller = new PathFindController(Game1.player, this, new Point(@int, int2), int3, OnReachedBoatDeck)
				{
			...
			if (forceWarpTimer > 0)
			{
				...
				if (forceWarpTimer <= 0)
				{
					forceWarpTimer = 0;
					Game1.player.controller = null;
					Game1.player.setTileLocation(new Vector2(@int, int2));
					Game1.player.faceDirection(int3);
