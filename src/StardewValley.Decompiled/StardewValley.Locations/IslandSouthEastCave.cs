...
public class IslandSouthEastCave : IslandLocation
{
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		{
			if (Game1.player.CurrentTool != null && Game1.player.CurrentTool is FishingRod fishingRod && (fishingRod.pullingOutOfWater || fishingRod.fishCaught || fishingRod.showingTreasure))
			{
				return;
			}
	...
	public override void TransferDataFromSavedLocation(GameLocation l)
	{
		...
		drinksClaimed.Clear();
		foreach (long item in islandSouthEastCave.drinksClaimed)
		{
			drinksClaimed.Add(item);
		}
	}
}
