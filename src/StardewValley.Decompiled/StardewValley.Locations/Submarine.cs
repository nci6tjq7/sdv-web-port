...
public class Submarine : GameLocation
{
	...
	public override void performTenMinuteUpdate(int timeOfDay)
	{
		...
		Random random = Utility.CreateDaySaveRandom(timeOfDay);
		if (fishSplashPoint.Value.Equals(Point.Zero) && random.NextDouble() < 1.0 && curtainOpenPercent >= 1f)
		{
			for (int i = 0; i < 2; i++)
			{
				Point value = new Point(random.Next(9, 21), random.Next(7, 12));
				if (!isOpenWater(value.X, value.Y))
				{
					continue;
				}
		...
		else if (!fishSplashPoint.Value.Equals(Point.Zero) && random.NextDouble() < 0.25)
		{
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (submergeTimer > 0f)
		{
			...
			else
			{
				...
				{
					Vector2 vector = new Vector2(Game1.random.Next(12, map.DisplayWidth - 64), ascending.Value ? 1 : 640);
					int num2 = Game1.random.Next(3);
					Game1.background.tempSprites.Add(new TemporaryAnimatedSprite
					{
						motion = new Vector2(0f, (float)((!ascending.Value) ? 1 : (-1)) * (-3f + (float)num2)),
						yStopCoordinate = ((!ascending.Value) ? 1 : 832),
						texture = submarineSprites,
						sourceRect = new Microsoft.Xna.Framework.Rectangle(132 + num2 * 8, 20, 8, 8),
						xPeriodic = true,
		...
		else if (submerged.Value && !doneUntilReset)
		{
			...
			{
				Vector2 vector2 = new Vector2(Game1.random.Next(384, map.DisplayWidth - 64), 320f);
				int num3 = Game1.random.Next(3);
				Game1.background.tempSprites.Add(new TemporaryAnimatedSprite
				{
					motion = new Vector2(0f, -1f + (float)num3 * 0.2f),
					yStopCoordinate = 1,
					texture = submarineSprites,
					sourceRect = new Microsoft.Xna.Framework.Rectangle(132 + num3 * 8, 20, 8, 8),
					animationLength = 1,
					interval = 20000f,
					xPeriodic = true,
					xPeriodicLoopTime = 1500f,
			...
			{
				Vector2 vector3 = new Vector2(1344f, Game1.random.Next(448, 704));
				Game1.background.tempSprites.Add(new TemporaryAnimatedSprite
				{
			...
			{
				Vector2 vector4 = new Vector2(3f, 10f) * 64f;
				Game1.background.tempSprites.Add(new TemporaryAnimatedSprite
				{
					motion = new Vector2(-0f, -1f),
			...
			{
				Vector2 vector5 = new Vector2(24f, 2f) * 64f;
				...
				Game1.background.tempSprites.Add(new TemporaryAnimatedSprite
				{
					...
					xStopCoordinate = 64,
					texture = submarineSprites,
					sourceRectStartingPos = new Vector2(257 + num4 * 48, 81f),
					sourceRect = new Microsoft.Xna.Framework.Rectangle(257 + num4 * 48, 81, 16, 16),
