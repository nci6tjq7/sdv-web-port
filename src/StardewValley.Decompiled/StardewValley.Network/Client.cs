...
public abstract class Client : IBandwidthMonitor
{
	...
	protected virtual void receiveAvailableFarmhands(BinaryReader msg)
	{
		...
		while (availableFarmhands.Count < num)
		{
			...
			Farmer value4 = netFarmerRoot.Value;
			availableFarmhands.Add(value4);
			value4.yearForSaveGame = value;
			value4.seasonForSaveGame = value2;
			value4.dayOfMonthForSaveGame = value3;
		}
		hasHandshaked = true;
		connectionMessage = null;
	...
	protected virtual void receiveServerIntroduction(BinaryReader msg)
	{
		...
		Game1.otherFarmers.Roots.Add(uniqueMultiplayerID, netFarmerRoot);
	...
	public virtual void sendMessages()
	{
		...
		foreach (OutgoingMessage item in Game1.serverHost.Value.messageQueue)
		{
			sendMessage(item);
		}
