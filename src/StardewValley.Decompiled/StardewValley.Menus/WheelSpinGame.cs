...
public class WheelSpinGame : IClickableMenu
{
	...
	public override void update(GameTime time)
	{
		...
		{
			double num = arrowRotationVelocity;
			arrowRotationVelocity += arrowRotationDeceleration;
			if (arrowRotationVelocity <= Math.PI / 80.0 && num > Math.PI / 80.0)
			{
				...
				else if ((arrowRotation + Math.PI) % (Math.PI * 2.0) <= 4.319689898685965 && !specialEventVariable && Game1.random.NextDouble() < (double)((float)Game1.player.LuckLevel / 20f))
				{
					arrowRotationVelocity = Math.PI / 48.0;
					Game1.playSound("dwop");
				}
			...
			double num2 = arrowRotation;
			arrowRotation += arrowRotationVelocity;
			if (num2 % (Math.PI / 2.0) > arrowRotation % (Math.PI / 2.0))
			{
