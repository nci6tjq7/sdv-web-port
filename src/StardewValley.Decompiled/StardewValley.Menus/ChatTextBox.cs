...
public class ChatTextBox : TextBox
{
	...
	public override void RecieveTextInput(string text)
	{
		...
		{
			ChatSnippet chatSnippet = finalText.Last();
	...
	public void updateWidth()
	{
		...
		foreach (ChatSnippet item in finalText)
		{
			...
			{
				item.myLength = ChatBox.messageFont(LocalizedContentManager.CurrentLanguageCode).MeasureString(item.message).X;
			}
			currentWidth += item.myLength;
		}
	}
	...
	public override void Draw(SpriteBatch spriteBatch, bool drawShadow = true)
	{
		...
		for (int i = 0; i < finalText.Count; i++)
		{
			...
			{
				spriteBatch.Draw(ChatBox.emojiTexture, new Vector2((float)base.X + num2 + 12f, base.Y + 12), new Rectangle(finalText[i].emojiIndex * 9 % ChatBox.emojiTexture.Width, finalText[i].emojiIndex * 9 / ChatBox.emojiTexture.Width * 9, 9, 9), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
			}
			else if (finalText[i].message != null)
			{
				spriteBatch.DrawString(ChatBox.messageFont(LocalizedContentManager.CurrentLanguageCode), finalText[i].message, new Vector2((float)base.X + num2 + 12f, base.Y + 12), ChatMessage.getColorFromName(Game1.player.defaultChatColor), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.99f);
			}
			num2 += finalText[i].myLength;
		}
	}
}
