...
public class TooManyFarmsMenu : IClickableMenu
{
	...
	{
		Vector2 topLeftPositionForCenteringOnScreen = Utility.getTopLeftPositionForCenteringOnScreen(800, 180);
		initialize((int)topLeftPositionForCenteringOnScreen.X, (int)topLeftPositionForCenteringOnScreen.Y, 800, 180);
	}
	...
	public override void draw(SpriteBatch b)
	{
		...
		SpriteText.drawString(b, s, xPositionOnScreen + num, yPositionOnScreen + num, 999999, width, height);
		int y = 260;
		Rectangle destinationRectangle = new Rectangle(xPositionOnScreen + width - 14 - 52, yPositionOnScreen + height - 14 - 52, 52, 52);
		Rectangle value = new Rectangle(542, y, 26, 26);
		if (Game1.options.gamepadControls)
		{
			b.Draw(Game1.controllerMaps, destinationRectangle, value, Color.White);
		}
	}
}
