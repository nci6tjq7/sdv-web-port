...
public class DayTimeMoneyBox : IClickableMenu
{
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (Game1.options.zoomButtons)
		{
			...
			{
				int num = (int)Math.Round(Game1.options.desiredBaseZoomLevel * 100f);
				num -= num % 5;
				num += 5;
				Game1.options.desiredBaseZoomLevel = Math.Min(2f, (float)num / 100f);
				Game1.forceSnapOnNextViewportUpdate = true;
				Game1.playSound("drumkit6");
			}
			else if (zoomOutButton.containsPoint(x, y) && Game1.options.desiredBaseZoomLevel > 0.75f)
			{
				int num2 = (int)Math.Round(Game1.options.desiredBaseZoomLevel * 100f);
				num2 -= num2 % 5;
				num2 -= 5;
				Game1.options.desiredBaseZoomLevel = Math.Max(0.75f, (float)num2 / 100f);
				Game1.forceSnapOnNextViewportUpdate = true;
	...
	public void pingQuest(Quest quest)
	{
		...
		if (!(quest is SlayMonsterQuest slayMonsterQuest))
		{
			if (!(quest is ResourceCollectionQuest resourceCollectionQuest))
			{
			...
			{
				ParsedItemData data2 = ItemRegistry.GetData(resourceCollectionQuest.ItemId.Value);
				...
				{
					questPingString = resourceCollectionQuest.numberCollected.Value + "/" + resourceCollectionQuest.number.Value;
				}
			}
		}
		else if (slayMonsterQuest.monster.Value?.Sprite?.Texture != null && slayMonsterQuest.monsterName != null)
		{
			questPingTexture = slayMonsterQuest.monster.Value.Sprite.Texture;
			questPingSourceRect = new Rectangle(0, 5, 16, 16);
			if (slayMonsterQuest.monsterName.Equals("Green Slime"))
			{
			...
			else if (slayMonsterQuest.monsterName.Equals("Squid Kid"))
			{
				questPingSourceRect = new Rectangle(0, 0, 16, 16);
			}
			if (slayMonsterQuest.numberToKill.Value != slayMonsterQuest.numberKilled.Value)
			{
				questPingString = slayMonsterQuest.numberKilled.Value + "/" + slayMonsterQuest.numberToKill.Value;
			}
		}
		else
		{
	...
	public override void draw(SpriteBatch b)
	{
		...
		Vector2 vector = spriteFont.MeasureString(_dateText);
		Vector2 vector2 = new Vector2((float)sourceRect.X * 0.5625f - vector.X / 2f, (float)sourceRect.Y * (LocalizedContentManager.CurrentLanguageLatin ? 0.1f : 0.1f) - vector.Y / 2f);
		Utility.drawTextWithShadow(b, _dateText, spriteFont, position + vector2, Game1.textColor);
		...
		Vector2 vector3 = spriteFont.MeasureString(_timeText);
		Vector2 vector4 = new Vector2((float)sourceRect.X * 0.55f - vector3.X / 2f + (float)((timeShakeTimer > 0) ? Game1.random.Next(-2, 3) : 0), (float)sourceRect.Y * (LocalizedContentManager.CurrentLanguageLatin ? 0.31f : 0.31f) - vector3.Y / 2f + (float)((timeShakeTimer > 0) ? Game1.random.Next(-2, 3) : 0));
		bool flag = Game1.shouldTimePass() || Game1.fadeToBlack || Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 2000.0 > 1000.0;
		Utility.drawTextWithShadow(b, _timeText, spriteFont, position + vector4, (Game1.timeOfDay >= 2400) ? Color.Red : (Game1.textColor * (flag ? 1f : 0.5f)));
		...
		if (Game1.player.hasVisibleQuests)
		{
			...
			if (questPulseTimer > 0)
			{
				...
				b.Draw(Game1.mouseCursors, new Vector2(questButton.bounds.X + 24, questButton.bounds.Y + 32) + ((num2 > 1f) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Rectangle(395, 497, 3, 8), Color.White, 0f, new Vector2(2f, 4f), 4f * num2, SpriteEffects.None, 0.99f);
			}
			if (questPingTimer > 0)
			{
		...
		b.Draw(Game1.mouseCursors, position + new Vector2(88f, 88f), new Rectangle(324, 477, 7, 19), Color.White, (float)(Math.PI + Math.Min(Math.PI, (double)(((float)num + (float)Game1.gameTimeInterval / (float)Game1.realMilliSecondsPerGameTenMinutes * 16.6f - 600f) / 2000f) * Math.PI)), new Vector2(3f, 17f), 4f, SpriteEffects.None, 0.9f);
		if (questNotificationTimer > 0)
		{
			Vector2 vector5 = position + new Vector2(27f, 76f) * 4f;
			b.Draw(Game1.mouseCursors_1_6, vector5, new Rectangle(257, 228, 39, 18), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.9f);
			b.Draw(questPingTexture, vector5 + new Vector2(1f, 1f) * 4f, questPingSourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.91f);
			if (questPingString != null)
			{
				Utility.drawTextWithShadow(b, questPingString, Game1.smallFont, vector5 + new Vector2(27f, 9.5f) * 4f - Game1.smallFont.MeasureString(questPingString) * 0.5f, Game1.textColor);
			}
			else
			{
