...
public class NumberSelectionMenu : IClickableMenu
{
	...
	public NumberSelectionMenu(string message, behaviorOnNumberSelect behaviorOnSelection, int price = -1, int minValue = 0, int maxValue = 99, int defaultNumber = 0)
	{
		...
		int x = (int)centerPosition.X - num / 2;
		int y = (int)centerPosition.Y - num2 / 2;
	...
	public override void gamePadButtonHeld(Buttons b)
	{
		...
		int num = (int)Math.Pow(10.0, (heldTimer - 300) / 3000);
		switch (currentlySnappedComponent.myID)
		{
		case 102:
		{
			int val2 = currentValue + num;
			int val3 = int.MaxValue;
			if (price != -1 && price != 0)
			{
				val3 = Game1.player.Money / price;
			}
			val2 = Math.Min(val2, Math.Min(maxValue, val3));
			if (val2 != currentValue)
			{
