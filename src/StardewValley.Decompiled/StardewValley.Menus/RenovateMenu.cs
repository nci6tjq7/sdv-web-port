...
public class RenovateMenu : IClickableMenu
{
		...
		if (_buildAnimationCount > 0)
		{
			...
			if (_renovation.animationType == HouseRenovation.AnimationType.Destroy)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					...
					int num = (int)Utility.RandomFloat((rectangle.Left - 1) * 64, 64 * rectangle.Right);
					int num2 = (int)Utility.RandomFloat((rectangle.Top - 1) * 64, 64 * rectangle.Bottom);
					_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, new Vector2(num, num2), flicker: false, Game1.random.NextBool()));
					_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, new Vector2(num, num2), flicker: false, Game1.random.NextBool()));
			...
			else
			{
				...
				for (int j = 0; j < 20; j++)
				{
					...
					int num3 = (int)Utility.RandomFloat((rectangle2.Left - 1) * 64, 64 * rectangle2.Right);
					int num4 = (int)Utility.RandomFloat((rectangle2.Top - 1) * 64, 64 * rectangle2.Bottom);
					_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90) - 64, 6, 1, new Vector2(num3, num4), flicker: false, Game1.random.NextBool()));
					_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90) - 64, 6, 1, new Vector2(num3, num4), flicker: false, Game1.random.NextBool()));
				}
			}
		}
	...
	public virtual void CompleteRenovation(int selected_index)
	{
		...
		bool flag = _renovation.Price < 0;
		if (!flag || Game1.player.mailReceived.Contains("FirstPurchase_" + _renovation.RoomId))
		{
			if (flag)
			{
