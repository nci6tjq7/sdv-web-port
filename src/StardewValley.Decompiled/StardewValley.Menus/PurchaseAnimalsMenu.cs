...
public class PurchaseAnimalsMenu : IClickableMenu
{
		...
		: base(Game1.uiViewport.Width / 2 - menuWidth / 2 - IClickableMenu.borderWidth * 2, (Game1.uiViewport.Height - menuHeight - IClickableMenu.borderWidth * 2) / 4, menuWidth + IClickableMenu.borderWidth * 2 + ((GetOffScreenRows(stock.Count) > 0) ? 44 : 0), menuHeight + IClickableMenu.borderWidth)
	{
		...
		for (int i = 0; i < stock.Count; i++)
		{
			...
			if (Game1.farmAnimalData.TryGetValue(stock[i].Name, out var value) && value.ShopTexture != null)
			{
				texture = Game1.content.Load<Texture2D>(value.ShopTexture);
				sourceRect = value.ShopSourceRect;
			}
			else if (i >= 9)
			{
		...
		int x = xPositionOnScreen + width - 64 - 24;
		upArrow = new ClickableTextureComponent(new Microsoft.Xna.Framework.Rectangle(x, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 16, 44, 48), Game1.mouseCursors, new Microsoft.Xna.Framework.Rectangle(421, 459, 11, 12), 4f)
		{
			myID = 105,
			rightNeighborID = -99998,
			leftNeighborID = -99998,
		...
		downArrow = new ClickableTextureComponent(new Microsoft.Xna.Framework.Rectangle(x, yPositionOnScreen + height - 64 - 24, 44, 48), Game1.mouseCursors, new Microsoft.Xna.Framework.Rectangle(421, 472, 11, 12), 4f)
		{
			myID = 106,
			rightNeighborID = -99998,
			leftNeighborID = -99998,
	...
	public virtual void RepositionAnimalButtons()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				int num = (i + currentScroll) * 3 + j;
				if (num >= animalsToPurchase.Count || num < 0)
				{
					break;
				}
				ClickableTextureComponent clickableTextureComponent = animalsToPurchase[num];
				clickableTextureComponent.bounds.X = xPositionOnScreen + IClickableMenu.borderWidth + j * 64 * 2;
				clickableTextureComponent.bounds.Y = yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth / 2 + i * 85;
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
			...
			if (!namingAnimal && buildingAt?.GetIndoors() is AnimalHouse animalHouse && !buildingAt.isUnderConstruction())
			{
				if (animalBeingPurchased.CanLiveIn(buildingAt))
				{
					...
					else
					{
						...
						FarmAnimalData animalData = animalBeingPurchased.GetAnimalData();
						if (animalData != null)
						{
		...
		foreach (ClickableTextureComponent item in animalsToPurchase)
		{
			if (readOnly || !item.containsPoint(x, y) || (item.item as Object).Type != null)
			{
				continue;
			}
			...
			if (Game1.player.Money >= num)
			{
				...
				animalBeingPurchased = new FarmAnimal(text, Game1.multiplayer.getNewID(), Game1.player.UniqueMultiplayerID);
				priceOfAnimal = num;
			}
			else
			{
	...
	public override void performHoverAction(int x, int y)
	{
		...
		if (onFarm)
		{
			if (!namingAnimal)
			{
				...
				Building buildingAt = targetLocation.getBuildingAt(tile);
				if (buildingAt?.GetIndoors() is AnimalHouse animalHouse)
				{
					if (animalBeingPurchased.CanLiveIn(buildingAt) && !animalHouse.isFull())
					{
		...
		foreach (ClickableTextureComponent item in animalsToPurchase)
		{
			...
			{
				item.scale = Math.Min(item.scale + 0.05f, 4.1f);
				hovered = item;
			}
			else
			{
				item.scale = Math.Max(4f, item.scale - 0.025f);
			}
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		if (!onFarm && !Game1.dialogueUp && !Game1.IsFading())
		{
			...
			foreach (ClickableTextureComponent item in animalsToPurchase)
			{
				item.draw(b, ((item.item as Object).Type != null) ? (Color.Black * 0.4f) : Color.White, 0.87f);
			}
		}
		...
		if (hovered != null)
		{
			...
			else
			{
				...
				string shopDescription = FarmAnimal.GetShopDescription(hovered.hoverText);
				IClickableMenu.drawHoverText(b, Game1.parseText(shopDescription, Game1.smallFont, 320), Game1.smallFont, 0, 0, hovered.item.salePrice(), displayName);
			}
		}
	...
	{
		Vector2 topLeftPositionForCenteringOnScreen = Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, building.tilesWide.Value * 64, building.tilesHigh.Value * 64);
		int x = building.tileX.Value * 64 - (int)topLeftPositionForCenteringOnScreen.X;
		int y = building.tileY.Value * 64 - (int)topLeftPositionForCenteringOnScreen.Y;
