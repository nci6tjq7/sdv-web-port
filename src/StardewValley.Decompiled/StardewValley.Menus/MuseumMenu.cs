...
public class MuseumMenu : MenuWithInventory
{
	...
	public override void receiveKeyPress(Keys key)
	{
		...
		else
		{
			...
			Vector2 vector = new Vector2((int)((Utility.ModifyCoordinateFromUIScale(Game1.getMouseX()) + (float)Game1.viewport.X) / 64f), (int)((Utility.ModifyCoordinateFromUIScale(Game1.getMouseY()) + (float)Game1.viewport.Y) / 64f));
			if (!museum.isTileSuitableForMuseumPiece((int)vector.X, (int)vector.Y) && (!reOrganizing || !LibraryMuseum.HasDonatedArtifactAt(vector)))
			{
				vector = museum.getFreeDonationSpot();
			...
			{
				vector = museum.findMuseumPieceLocationInDirection(vector, 0, 21, !reOrganizing);
			}
			else if (key == Game1.options.getFirstKeyboardKeyFromInputButtonList(Game1.options.moveRightButton))
			{
				vector = museum.findMuseumPieceLocationInDirection(vector, 1, 21, !reOrganizing);
			}
			else if (key == Game1.options.getFirstKeyboardKeyFromInputButtonList(Game1.options.moveDownButton))
			{
				vector = museum.findMuseumPieceLocationInDirection(vector, 2, 21, !reOrganizing);
			}
			else if (key == Game1.options.getFirstKeyboardKeyFromInputButtonList(Game1.options.moveLeftButton))
			{
				vector = museum.findMuseumPieceLocationInDirection(vector, 3, 21, !reOrganizing);
			}
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (!holdingMuseumPiece)
		{
			if (base.heldItem == null)
			{
				...
				Item item2 = ((inventoryPositionOfClick >= 0 && inventoryPositionOfClick < inventory.actualInventory.Count) ? inventory.actualInventory[inventoryPositionOfClick] : null);
				if (item2 != null && inventory.highlightMethod(item2))
				{
					base.heldItem = item2.getOne();
					inventory.actualInventory[inventoryPositionOfClick] = item2.ConsumeStack(1);
				}
			}
			else
			{
		...
		if (item == null && base.heldItem != null && Game1.isAnyGamePadButtonBeingPressed())
		{
			receiveGamePadButton(Buttons.DPadUp);
		}
		if (item != null && base.heldItem != null && (y < Game1.viewport.Height - (height - (IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 192)) || menuMovingDown))
		{
			Item item3 = base.heldItem;
			LibraryMuseum museum = Museum;
			int num = (int)(Utility.ModifyCoordinateFromUIScale(x) + (float)Game1.viewport.X) / 64;
			int num2 = (int)(Utility.ModifyCoordinateFromUIScale(y) + (float)Game1.viewport.Y) / 64;
			if (museum.isTileSuitableForMuseumPiece(num, num2) && museum.isItemSuitableForDonation(item3))
			{
				...
				museum.museumPieces.Add(new Vector2(num, num2), item3.ItemId);
				Game1.playSound("stoneStep");
				if (museum.getRewardsForPlayer(Game1.player).Count > count && !holdingMuseumPiece)
				{
				...
				if (!holdingMuseumPiece)
				{
					...
					{
						Game1.multiplayer.globalChatInfoMessage("donation", Game1.player.name.Value, TokenStringBuilder.ItemNameFor(item3));
					}
				}
				ReturnToDonatableItems();
			}
		...
		{
			int num3 = (int)(Utility.ModifyCoordinateFromUIScale(x) + (float)Game1.viewport.X) / 64;
			int num4 = (int)(Utility.ModifyCoordinateFromUIScale(y) + (float)Game1.viewport.Y) / 64;
			Vector2 key = new Vector2(num3, num4);
			LibraryMuseum museum2 = Museum;
			if (museum2.museumPieces.TryGetValue(key, out var value))
			{
				base.heldItem = ItemRegistry.Create(value, 1, 0, allowNull: true);
				museum2.museumPieces.Remove(key);
				if (base.heldItem != null)
				{
		...
		if (base.heldItem != null && item == null)
		{
			menuMovingDown = true;
			reOrganizing = false;
		}
	...
	public override void emergencyShutDown()
	{
		...
		{
			Vector2 freeDonationSpot = Museum.getFreeDonationSpot();
			if (Museum.museumPieces.TryAdd(freeDonationSpot, base.heldItem.ItemId))
			{
				base.heldItem = null;
				holdingMuseumPiece = false;
			}
	...
	public override void update(GameTime time)
	{
		...
		if ((!Game1.options.SnappyMenus && Game1.lastCursorMotionWasMouse && num - Game1.viewport.X < 64) || Game1.input.GetGamePadState().ThumbSticks.Right.X < 0f)
		{
			Game1.panScreen(-4, 0);
		...
		else if ((!Game1.options.SnappyMenus && Game1.lastCursorMotionWasMouse && num - (Game1.viewport.X + Game1.viewport.Width) >= -64) || Game1.input.GetGamePadState().ThumbSticks.Right.X > 0f)
		{
			Game1.panScreen(4, 0);
			if (Game1.input.GetGamePadState().ThumbSticks.Right.X > 0f)
			{
		...
		if ((!Game1.options.SnappyMenus && Game1.lastCursorMotionWasMouse && num2 - Game1.viewport.Y < 64) || Game1.input.GetGamePadState().ThumbSticks.Right.Y > 0f)
		{
			Game1.panScreen(0, -4);
		...
		else if ((!Game1.options.SnappyMenus && Game1.lastCursorMotionWasMouse && num2 - (Game1.viewport.Y + Game1.viewport.Height) >= -64) || Game1.input.GetGamePadState().ThumbSticks.Right.Y < 0f)
		{
			Game1.panScreen(0, 4);
			if (Game1.input.GetGamePadState().ThumbSticks.Right.Y < 0f)
			{
		...
		if ((fadeTimer <= 0 || !fadeIntoBlack) && state != 3)
		{
			if (base.heldItem != null)
			{
				...
				for (int i = Game1.viewport.Y / 64 - 1; i < (Game1.viewport.Y + Game1.viewport.Height) / 64 + 2; i++)
				{
					...
					{
						if (Museum.isTileSuitableForMuseumPiece(j, i))
						{
