...
public class MenuWithInventory : IClickableMenu
{
		...
		: base(Game1.uiViewport.Width / 2 - (800 + IClickableMenu.borderWidth * 2) / 2, Game1.uiViewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2 + menuOffsetHack, 800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2)
	{
		...
		int num = yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth + 192 - 16 + inventoryYOffset;
		inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2 + inventoryXOffset, num, playerInventory: false, null, highlighterMethod);
		...
		dropItemInvisibleButton = new ClickableComponent(new Rectangle(xPositionOnScreen - IClickableMenu.borderWidth - IClickableMenu.spaceToClearSideBorder - 128, num - 12, 64, 64), "")
		{
			myID = 107,
			rightNeighborID = 0
		};
	...
	public virtual void DropHeldItem()
	{
		if (heldItem != null)
		{
			...
			Game1.createItemDebris(heldItem, Game1.player.getStandingPosition(), direction);
	...
	public virtual void draw(SpriteBatch b, bool drawUpperPortion = true, bool drawDescriptionArea = true, int red = -1, int green = -1, int blue = -1)
	{
		...
		if (drawUpperPortion)
		{
			...
			if (drawDescriptionArea)
			{
				...
				{
					int num = xPositionOnScreen + 576 + 42 + ((wiggleWordsTimer > 0) ? Game1.random.Next(-2, 3) : 0);
					int num2 = yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder - 32 + ((wiggleWordsTimer > 0) ? Game1.random.Next(-2, 3) : 0);
					int num3 = 320;
					float num4 = 0f;
					string text;
					do
					{
						num4 = ((num4 != 0f) ? (num4 - 0.1f) : 1f);
						text = Game1.parseText(descriptionText, Game1.smallFont, (int)(224f / num4));
					}
					while (Game1.smallFont.MeasureString(text).Y > (float)num3 / num4 && num4 > 0.5f);
					if (red == -1)
					{
						Utility.drawTextWithShadow(b, text, Game1.smallFont, new Vector2(num, num2), Game1.textColor * 0.75f, num4);
					}
					else
					{
						Utility.drawTextWithColoredShadow(b, text, Game1.smallFont, new Vector2(num, num2), Game1.textColor * 0.75f, Color.Black * 0.2f, num4);
					}
				}
			}
		}
	...
	public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
	{
		...
		int yPosition = yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth + 192 - 16;
		string moveItemSound = inventory.moveItemSound;
		inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2, yPosition, playerInventory: false, null, inventory.highlightMethod);
		inventory.moveItemSound = moveItemSound;
		if (okButton != null)
		{
