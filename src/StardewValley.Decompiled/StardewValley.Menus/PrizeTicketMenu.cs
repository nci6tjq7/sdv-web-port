...
public class PrizeTicketMenu : IClickableMenu
{
	...
	{
		Random random = Utility.CreateRandom(Game1.uniqueIDForThisGame, Game1.player.UniqueMultiplayerID);
		switch (prizeLevel)
		{
		case 0:
			return Utility.getRaccoonSeedForCurrentTimeOfYear(Game1.player, random, 12);
		case 1:
			return ItemRegistry.Create(random.Choose("(O)631", "(O)630"));
		case 2:
			return random.Choose(ItemRegistry.Create("(O)770", 10), ItemRegistry.Create("(O)MixedFlowerSeeds", 15));
		case 3:
		...
		case 6:
			return ItemRegistry.Create(random.Choose("(O)621", "(BC)15", "(BC)MushroomLog"), 4);
		case 7:
			return ItemRegistry.Create(random.Choose("(O)633", "(O)632"));
		case 8:
			return ItemRegistry.Create("(O)Book_Friendship");
		case 9:
			return random.Choose(ItemRegistry.Create("(O)286", 20), ItemRegistry.Create("(O)287", 12), ItemRegistry.Create("(O)288", 6));
		case 10:
			return ItemRegistry.Create("(H)SportsCap");
		case 11:
			return ItemRegistry.Create(random.Choose("(BC)FishSmoker", "(BC)Dehydrator"));
		case 12:
			return ItemRegistry.Create(random.Choose("(O)275", "(O)MysteryBox"), 4);
		case 13:
			return ItemRegistry.Create(random.Choose("(F)FancyHousePlant1", "(F)FancyHousePlant2", "(F)FancyHousePlant3"));
		case 14:
		...
		case 18:
			return ItemRegistry.Create(random.Choose("(BC)10", "(BC)12"), 4);
		case 19:
		...
		default:
		{
			Random random2 = Utility.CreateRandom(Game1.uniqueIDForThisGame, prizeLevel - prizeLevel % 9);
			return (prizeLevel % 9) switch
			{
				0 => ItemRegistry.Create("(O)MysteryBox", 5), 
				1 => ItemRegistry.Create("(O)872", random2.Next(1, 3)), 
				2 => ItemRegistry.Create(random2.Choose<string>("(O)337", "(O)226", "(O)253", "(O)732", "(O)275"), 5), 
				3 => ItemRegistry.Create(random2.Choose("(F)FancyHousePlant1", "(F)FancyHousePlant2", "(F)FancyHousePlant3")), 
				4 => ItemRegistry.Create("(O)StardropTea"), 
				5 => ItemRegistry.Create("(O)166"), 
				6 => ItemRegistry.Create("(O)645"), 
				7 => ItemRegistry.Create(random2.Choose("(F)FancyTree1", "(F)FancyTree2", "(F)FancyTree3", "(F)PigPainting")), 
				8 => random2.Choose(ItemRegistry.Create("(O)287", 15), ItemRegistry.Create("(O)288", 8)), 
	...
	public override void draw(SpriteBatch b)
	{
		...
		{
			Vector2 vector = new Vector2(28 + 22 * i, 21f) * 4f;
			if (movingRewardTrack)
			{
				float num = 88f - moveRewardTrackTimer / 18f;
				if (num > 0f)
				{
					vector.X += num;
					if (moveRewardTrackPreTimer <= 0f)
					{
						vector.X += Game1.random.Next(-1, 2);
						vector.Y += Game1.random.Next(-1, 2);
					}
				}
			}
			if (i == 0)
			...
			{
				currentPrizeTrack[i].drawInMenu(b, base.Position + vector, 1f);
			}
		}
		...
		if (gettingReward)
		{
			...
			vector2.Y -= getRewardTimer / 13f;
			vector2.Y = Math.Max(vector2.Y, 0f);
			vector2.X += getRewardTimer / 1000f * (float)Game1.random.Next(-1, 2);
			vector2.Y += getRewardTimer / 1000f * (float)Game1.random.Next(-1, 2);
			currentPrizeTrack[0].drawInMenu(b, base.Position + vector2, 1f, 1f, 0.9f, StackDrawType.Draw, Color.White, drawShadow: false);
		}
		string s = Game1.player.Items.CountId("PrizeTicket").ToString() ?? "";
		SpriteText.drawString(b, s, xPositionOnScreen + 360 - SpriteText.getWidthOfString(s) / 2, yPositionOnScreen + 276);
		mainButton.draw(b);
		base.draw(b);
		drawMouse(b);
	}
