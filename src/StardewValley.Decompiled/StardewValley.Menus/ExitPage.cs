...
public class ExitPage : IClickableMenu
{
	...
	{
		string text = Game1.content.LoadString("Strings\\UI:ExitToTitle");
		if (!Game1.game1.IsMainInstance)
		{
			text = Game1.content.LoadString("Strings\\UI:DropOutLocalMulti");
		}
		Vector2 vector = new Vector2(xPositionOnScreen + width / 2 - (int)((Game1.dialogueFont.MeasureString(text).X + 64f) / 2f), yPositionOnScreen + 256 - 32);
		exitToTitle = new ClickableComponent(new Rectangle((int)vector.X, (int)vector.Y, (int)Game1.dialogueFont.MeasureString(text).X + 64, 96), "", text)
		{
			myID = 535,
			upNeighborID = 12349,
			downNeighborID = 536
		};
		vector = new Vector2(xPositionOnScreen + width / 2 - (int)((Game1.dialogueFont.MeasureString(Game1.content.LoadString("Strings\\UI:ExitToDesktop")).X + 64f) / 2f), yPositionOnScreen + 384 + 8 - 32);
		exitToDesktop = new ClickableComponent(new Rectangle((int)vector.X, (int)vector.Y, (int)Game1.dialogueFont.MeasureString(Game1.content.LoadString("Strings\\UI:ExitToDesktop")).X + 64, 96), "", Game1.content.LoadString("Strings\\UI:ExitToDesktop"))
		{
			myID = 536,
			upNeighborID = 535
		};
