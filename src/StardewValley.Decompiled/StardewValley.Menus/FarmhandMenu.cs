...
public class FarmhandMenu : LoadGameMenu
{
	public class FarmhandSlot : SaveFileSlot
	{
		...
		protected override void drawSlotName(SpriteBatch b, int i)
		{
			...
			SpriteText.drawString(b, s, menu.slotButtons[i].bounds.X + 128 + 36, menu.slotButtons[i].bounds.Y + 36);
		}
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		{
			if (slotButtons[i].containsPoint(x, y) && i < MenuSlots.Count && MenuSlots[currentItemIndex + i] is FarmhandSlot farmhandSlot && farmhandSlot.BelongsToAnotherPlayer())
			{
