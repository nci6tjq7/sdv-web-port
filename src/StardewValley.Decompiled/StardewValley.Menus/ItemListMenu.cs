...
public class ItemListMenu : IClickableMenu
{
	...
	public ItemListMenu(string menuTitle, List<Item> itemList)
	{
		...
		{
			totalValueOfItems += Utility.getSellToStorePriceOfItem(item);
		}
		itemsToList.Add(null);
		int num = Game1.uiViewport.Width / 2;
		...
		xPositionOnScreen = num - width / 2;
		yPositionOnScreen = num2 - height / 2;
		Rectangle bounds = new Rectangle(num + width / 2 + 4, num2 + height / 2 - 96, 64, 64);
		okButton = new ClickableTextureComponent(Game1.content.LoadString("Strings\\StringsFromCSFiles:ShippingMenu.cs.11382"), bounds, null, Game1.content.LoadString("Strings\\StringsFromCSFiles:ShippingMenu.cs.11382"), Game1.mouseCursors, new Rectangle(128, 256, 64, 64), 1f)
		{
			myID = 101,
			leftNeighborID = -7777
		};
	...
	public override void draw(SpriteBatch b)
	{
		...
		Vector2 location = new Vector2(xPositionOnScreen + 32, yPositionOnScreen + 96 + 4);
		for (int i = currentTab * itemsPerCategoryPage; i < currentTab * itemsPerCategoryPage + itemsPerCategoryPage; i++)
		{
			...
			if (itemsToList[i] == null)
			{
				...
				{
					SpriteText.drawString(b, Game1.content.LoadString("Strings\\UI:ItemList_ItemsLostValue", totalValueOfItems), (int)location.X + 64 + 12, (int)location.Y + 12);
				}
			}
			else
			{
				itemsToList[i].drawInMenu(b, location, 1f, 1f, 1f, StackDrawType.Draw_OneInclusive);
				SpriteText.drawString(b, itemsToList[i].DisplayName, (int)location.X + 64 + 12, (int)location.Y + 12);
				location.Y += 68f;
			}
		}
		if (showBackButton())
