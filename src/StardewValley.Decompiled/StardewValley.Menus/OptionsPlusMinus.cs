...
public class OptionsPlusMinus : OptionsElement
{
		...
		: base(label, x, y, 28, 28, whichOption)
	{
		...
		int num = (int)Game1.dialogueFont.MeasureString(options[0]).X + 28;
		foreach (string displayOption in displayOptions)
		{
			num = Math.Max((int)Game1.dialogueFont.MeasureString(displayOption).X + 28, num);
		}
		bounds = new Rectangle(x, y, 56 + num, 32);
		base.label = label;
