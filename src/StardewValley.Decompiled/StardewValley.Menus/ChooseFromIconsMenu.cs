...
public class ChooseFromIconsMenu : IClickableMenu
{
	...
	public void setUpIcons(string which)
	{
		...
		if (!(which == "dwarfStatue"))
		{
			if (which == "bobbers")
			{
				...
				int num4 = Game1.player.fishCaught.Count() / 2;
				num = 4;
				...
				{
					bool num5 = i > num4;
					Rectangle sourceRectForStandardTileSheet = Game1.getSourceRectForStandardTileSheet(Game1.bobbersTexture, i, 16, 32);
					...
					{
						iconFronts.Add(new ClickableTextureComponent(new Rectangle(0, 0, 16, 16), Game1.bobbersTexture, sourceRectForStandardTileSheet, 4f, drawShadow: true));
					}
				}
		...
		else
		{
			...
			texture = Game1.mouseCursors_1_6;
			Random random = Utility.CreateRandom(Game1.stats.DaysPlayed * 77, Game1.uniqueIDForThisGame);
			int num6 = random.Next(5);
			int num7 = -1;
			do
			{
				num7 = random.Next(5);
			...
			{
				name = (num6.ToString() ?? ""),
				hoverText = Game1.content.LoadString("Strings\\1_6_Strings:DwarfStatue_" + num6)
			});
			icons.Add(new ClickableTextureComponent(new Rectangle(0, 0, 84, 84), texture, iconBackRectangle, 4f, drawShadow: true)
			{
				name = (num7.ToString() ?? ""),
				hoverText = Game1.content.LoadString("Strings\\1_6_Strings:DwarfStatue_" + num7)
			});
			iconFronts.Add(new ClickableTextureComponent(new Rectangle(0, 0, 17, 17), texture, new Rectangle(148 + num6 * 17, 123, 17, 17), 4f));
			iconFronts.Add(new ClickableTextureComponent(new Rectangle(0, 0, 17, 17), texture, new Rectangle(148 + num7 * 17, 123, 17, 17), 4f));
		}
		int num8 = (hasTooltips ? 240 : 0);
		int num9 = Math.Max(iconBackRectangle.Width * 4, num8) + num;
		iconXOffset = num9 / 2 - iconBackRectangle.Width * 4 / 2 - 4;
		...
		if (hasTooltips)
		{
			...
			{
				icon.hoverText = Game1.parseText(icon.hoverText, Game1.smallFont, num8 - 32);
				maxTooltipHeight = Math.Max(maxTooltipHeight, (int)Game1.smallFont.MeasureString(icon.hoverText).Y);
				maxTooltipWidth = Math.Max(maxTooltipWidth, (int)Game1.smallFont.MeasureString(icon.hoverText).X);
			}
			maxTooltipHeight += 48;
			maxTooltipWidth += 48;
		}
		height += (icons.Count * num9 / width + 1) * (maxTooltipHeight + icons[0].bounds.Height + num);
		int num10 = width / num9;
		yPositionOnScreen = Game1.uiViewport.Height / 2 - height / 2;
		int num11 = yPositionOnScreen + 100;
		for (int j = 0; j < icons.Count; j += num10)
		{
			int num12 = Math.Min(icons.Count - j, num10);
			int num13 = xPositionOnScreen + width / 2 - num12 * num9 / 2;
			for (int k = 0; k < num12; k++)
			{
				int num14 = k + j;
				icons[num14].bounds.X = num13 + k * num9;
				icons[num14].bounds.Y = num11;
				icons[num14].bounds.Width = num9;
				icons[num14].bounds.Height += maxTooltipHeight;
				iconFronts[num14].bounds.X = icons[num14].bounds.X + num2;
				iconFronts[num14].bounds.Y = icons[num14].bounds.Y + num3;
				icons[num14].myID = num14;
				icons[num14].leftNeighborID = num14 - 1;
				icons[num14].rightNeighborID = num14 + 1;
				icons[num14].downNeighborID = num14 + num12;
				icons[num14].upNeighborID = num14 - num12;
			}
			num11 += maxTooltipHeight + icons[0].bounds.Height + num;
		}
	...
	public override void performHoverAction(int x, int y)
	{
		...
		{
			ClickableTextureComponent clickableTextureComponent = icons[i];
			iconFronts[i].sourceRect = iconFronts[i].startingSourceRect;
			if (clickableTextureComponent.containsPoint(x, y) && destroyTimer == -1f)
			{
				if (clickableTextureComponent.sourceRect == clickableTextureComponent.startingSourceRect && hoverSound != null)
				{
					Game1.playSound(hoverSound);
				}
				clickableTextureComponent.sourceRect.Location = iconBackHighlightPosition;
			...
			{
				clickableTextureComponent.sourceRect = iconBackRectangle;
			}
		}
	}
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		for (int i = 0; i < icons.Count; i++)
		{
			...
			bool flag = iconFronts[i].name.Contains("ghosted");
			string text = which;
			if (!(text == "dwarfStatue"))
			{
				if (text == "bobbers")
				{
					...
					int num = Convert.ToInt32(clickableTextureComponent.name);
					if (Game1.player.bobberStyle.Value != num)
					{
						Game1.playSound("button1");
						hoverSound = null;
						Game1.player.bobberStyle.Value = Convert.ToInt32(clickableTextureComponent.name);
						selected = Game1.player.bobberStyle.Value;
						if (selected == -2)
						{
			...
			else
			{
				...
				for (int j = 0; j < 16; j++)
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(98 + Game1.random.Next(3) * 4, 161, 4, 4), Utility.getRandomPositionInThisRectangle(clickableTextureComponent.bounds, Game1.random), flipped: false, 0f, Color.White)
					{
						local = true,
						scale = 4f,
						interval = 9999f,
			...
			doIconAction(clickableTextureComponent.name);
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < icons.Count; i++)
		{
			if (selected == i || (selected == -2 && i == icons.Count - 1))
			{
				if (selected == i)
				{
					Rectangle bounds = icons[i].bounds;
					bounds.Inflate(2, 4);
					bounds.X += iconXOffset - 2;
