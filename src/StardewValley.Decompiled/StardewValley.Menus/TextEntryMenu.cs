...
public class TextEntryMenu : IClickableMenu
{
		...
		: base((int)Utility.getTopLeftPositionForCenteringOnScreen(672, 352).X, (int)Utility.getTopLeftPositionForCenteringOnScreen(672, 352).Y, 672, 352)
	{
		...
		for (int i = 0; i < letterMaps[0].Length; i++)
		{
			...
			{
				ClickableTextureComponent clickableTextureComponent = new ClickableTextureComponent(new Rectangle(0, 0, 1024, 1024), Game1.mouseCursors2, new Rectangle(32, 176, 16, 16), 4f)
				{
				...
				if (i == letterMaps[0].Length - 1)
				{
					...
					{
						clickableTextureComponent.downNeighborID = 99991;
						clickableTextureComponent.downNeighborImmutable = true;
					}
					if (j >= _lettersPerRow - 3 && j <= _lettersPerRow - 2)
					{
						clickableTextureComponent.downNeighborID = 99990;
						clickableTextureComponent.downNeighborImmutable = true;
					}
				}
				keys.Add(clickableTextureComponent);
			}
		}
	...
	public void RepositionElements()
	{
		...
		for (int i = 0; i < keys.Count / _lettersPerRow; i++)
		{
			for (int j = 0; j < _lettersPerRow; j++)
			{
				keys[j + i * _lettersPerRow].bounds = new Rectangle(xPositionOnScreen + 16 + j * 16 * 4, yPositionOnScreen + 16 + i * 16 * 4, 64, 64);
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		{
			key.draw(b);
			Vector2 vector = Game1.dialogueFont.MeasureString(key.name);
			b.DrawString(Game1.dialogueFont, key.name, Utility.snapDrawPosition(new Vector2((float)key.bounds.Center.X - vector.X / 2f, (float)key.bounds.Center.Y - vector.Y / 2f)), Color.Black);
		}
