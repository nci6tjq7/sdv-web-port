	...
	public override void updateWhenCurrentLocation(GameTime time)
	{
		...
		if (location != null && location.IsMiniJukeboxPlaying())
		{
			...
			if (showNote)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(516, 1916, 7, 10), 9999f, 1, 1, tileLocation.Value * 64f + new Vector2(Game1.random.Next(48), -80f), flicker: false, flipped: false, (tileLocation.Value.Y + 1f) * 64f / 10000f, 0.01f, Color.White, 4f, 0f, 0f, 0f)
					{
						xPeriodic = true,
