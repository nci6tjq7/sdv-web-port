...
public class DefaultPhoneHandler : IPhoneHandler
{
	...
	public string CheckForIncomingCall(Random random)
	{
		...
		foreach (KeyValuePair<string, IncomingPhoneCallData> item in DataLoader.IncomingPhoneCalls(Game1.content))
		{
			if ((flag || item.Value.IgnoreBaseChance) && (item.Value.TriggerCondition == null || GameStateQuery.CheckConditions(item.Value.TriggerCondition, Game1.currentLocation, Game1.player, null, null, random)))
			{
				list.Add(item.Key);
			}
		}
	...
	public bool TryHandleIncomingCall(string callId, out Action showDialogue)
	{
		...
		if (call.MaxCalls > -1 && Game1.player.callsReceived.TryGetValue(callId, out var value) && value >= call.MaxCalls)
		{
			return false;
		}
