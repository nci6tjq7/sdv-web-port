...
public class ItemPedestal : Object
{
	...
	protected override void GetOneCopyFrom(Item source)
	{
		...
		{
			isIslandShrinePedestal.Value = itemPedestal.isIslandShrinePedestal.Value;
		}
	}
	...
	{
		Vector2 globalPosition = new Vector2(x * 64, y * 64);
		ParsedItemData dataOrErrorItem = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
		b.Draw(Game1.bigCraftableSpriteSheet, Game1.GlobalToLocal(Game1.viewport, globalPosition), dataOrErrorItem.GetSourceRect(), Color.White, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, Math.Max(0f, (globalPosition.Y - 2f) / 10000f));
		if (match.Value)
		{
			b.Draw(Game1.bigCraftableSpriteSheet, Game1.GlobalToLocal(Game1.viewport, globalPosition), dataOrErrorItem.GetSourceRect(1), successColor.Value, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, Math.Max(0f, (globalPosition.Y - 1f) / 10000f));
		}
		if (heldObject.Value != null)
		{
			...
			heldObject.Value.draw(b, (int)vector.X * 64, (int)((vector.Y - 0.2f) * 64f) - 64, globalPosition.Y / 10000f, 1f);
		}
	}
}
