...
public static class FrameworkExtensions
{
	...
	{
		return (map?.GetLayer(layerId)?.HasTileAt(tile.X, tile.Y, tilesheetId)).GetValueOrDefault();
	}
	...
	{
		return (map?.GetLayer(layerId)?.HasTileAt(x, y, tilesheetId)).GetValueOrDefault();
	}
	...
	{
		return (map?.GetLayer(layerId)?.GetTileIndexAt(x, y, tilesheetId)).GetValueOrDefault(-1);
	}
	...
	{
		return (map?.GetLayer(layerId)?.GetTileIndexAt(tile.X, tile.Y, tilesheetId)).GetValueOrDefault(-1);
	}
