...
public class Tree : TerrainFeature
{
		...
		if ((maxShake == 0f || doEvenIfStillShaking) && growthStage.Value >= 3 && !stump.Value)
		{
			...
			if (growthStage.Value >= 5)
			{
				if (IsLeafy())
				{
					...
					if (Game1.random.NextDouble() < 0.01 && (localSeason == Season.Spring || localSeason.GetValueOrDefault() == Season.Summer))
					{
	...
	{
		if (localSeason.GetValueOrDefault() == Season.Winter && !fertilized.Value && !Location.SeedsIgnoreSeasonsHere())
		{
	...
	public override void dayUpdate()
	{
		...
		if (localSeason.GetValueOrDefault() == Season.Winter && data != null && data.IsStumpDuringWinter && !Location.SeedsIgnoreSeasonsHere())
		{
			stump.Value = true;
		}
		...
		if (isTemporaryGreenRainTree.Value && location.IsGreenhouse && (localSeason.GetValueOrDefault() == Season.Winter || localSeason.GetValueOrDefault() == Season.Fall))
		{
			hasSeed.Value = false;
		}
		else
		...
		if (!location.IsGreenhouse && (localSeason.GetValueOrDefault() == Season.Winter || stump.Value))
		{
			hasMoss.Value = false;
			return;
		}
		bool? flag2 = data?.GrowsMoss;
		if (flag2.HasValue && flag2.GetValueOrDefault() && growthStage.Value >= 14 && !stump.Value && Game1.random.NextBool(num3))
		{
			hasMoss.Value = true;
		}
	}
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if (growthStage.Value >= 5)
		{
			...
			if (t is Axe)
			{
				...
				{
					Object @object = gameLocation.tryToCreateUnseenSecretNote(t.getLastFarmerToUse());
					if (@object != null)
					{
						Game1.createItemDebris(@object, new Vector2(tileLocation.X, tileLocation.Y - 3f) * 64f, -1, gameLocation, Game1.player.StandingPixel.Y - 32);
					}
				}
