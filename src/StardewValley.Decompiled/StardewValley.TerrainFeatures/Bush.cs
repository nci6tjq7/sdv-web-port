...
public class Bush : LargeTerrainFeature
{
	...
	public void setUpSourceRect()
	{
		...
		int num = (int)cosmeticSeason;
		switch (size.Value)
		{
		case 0:
			sourceRect.Value = new Rectangle(num * 16 * 2 + tileSheetOffset.Value * 16, 224, 16, 32);
			break;
		case 1:
		{
			if (townBush.Value)
			{
				sourceRect.Value = new Rectangle(num * 16 * 2, 96, 32, 32);
				break;
			}
			int num2 = num * 16 * 4 + tileSheetOffset.Value * 16 * 2;
			sourceRect.Value = new Rectangle(num2 % texture.Value.Bounds.Width, num2 / texture.Value.Bounds.Width * 3 * 16, 32, 48);
			break;
		}
		case 2:
			if (townBush.Value && (cosmeticSeason == Season.Spring || cosmeticSeason == Season.Summer))
			{
	...
	public bool inBloom()
	{
		...
		if (size.Value == 3)
		{
			bool flag = getAge() >= 20 && dayOfMonth >= 22 && (season != Season.Winter || IsSheltered());
			if (flag && Location != null && Location.IsFarm)
			{
	...
	{
		Vector2 tile = Tile;
		Random random = Utility.CreateRandom(Game1.stats.DaysPlayed, Game1.uniqueIDForThisGame, tile.X, (double)tile.Y * 777.0);
		double num = ((random.NextDouble() < 0.5) ? 0.0 : ((double)random.Next(6) / 100.0));
		if (size.Value != 4)
		{
			if (size.Value == 1 && !readyForHarvest() && random.NextDouble() < 0.2 + num && inBloom())
			{
				tileSheetOffset.Value = 1;
			}
	...
	public override Rectangle getBoundingBox()
	{
		...
		case 2:
			return new Rectangle((int)tile.X * 64, (int)tile.Y * 64, 192, 64);
		default:
	...
	public override Rectangle getRenderBounds()
	{
		...
		case 0:
		case 3:
			return new Rectangle((int)tile.X * 64, (int)(tile.Y - 1f) * 64, 64, 160);
		case 1:
		case 4:
			return new Rectangle((int)tile.X * 64, (int)(tile.Y - 2f) * 64, 128, 256);
		case 2:
			return new Rectangle((int)tile.X * 64, (int)(tile.Y - 2f) * 64, 192, 256);
		default:
			return Rectangle.Empty;
		}
	}
	...
	public void shake(Vector2 tileLocation, bool doEvenIfStillShaking)
	{
		...
		if (!townBush.Value && readyForHarvest() && inBloom())
		{
			...
			default:
			{
				...
				Game1.player.gainExperience(2, num);
				break;
			}
			}
			if (size.Value != 3)
	...
	public override void dayUpdate()
	{
		...
		if (size.Value != 4)
		{
			...
			setUpSourceRect();
			Vector2 tile = Tile;
			if (tile.X != 6f || tile.Y != 7f || !(location.Name == "Sunroom"))
			{
				health = 0f;
			}
		}
			...
			if (axe.upgradeLevel.Value >= 1 || size.Value == 3)
			{
				...
				if (health <= -1f)
				{
					...
					for (int i = 0; i <= getEffectiveSize(); i++)
					{
						for (int j = 0; j < 12; j++)
						{
							Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(355, 1200 + (season.Equals("fall") ? 16 : (season.Equals("winter") ? (-16) : 0)), 16, 16), Utility.getRandomPositionInThisRectangle(getBoundingBox(), Game1.random) - new Vector2(0f, Game1.random.Next(64)), flipped: false, 0.01f, color)
							{
							...
							if (j % 6 == 0)
							{
								Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(50, Utility.getRandomPositionInThisRectangle(getBoundingBox(), Game1.random) - new Vector2(32f, Game1.random.Next(32, 64)), color));
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		if (drawShadow.Value)
		{
			...
			{
				spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((tile.X + ((getEffectiveSize() == 1) ? 0.5f : 1f)) * 64f - 51f, tile.Y * 64f - 16f + yDrawOffset)), shadowSourceRect, Color.White, 0f, Vector2.Zero, 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 1E-06f);
			}
			else
			{
				spriteBatch.Draw(Game1.shadowTexture, Game1.GlobalToLocal(Game1.viewport, new Vector2(tile.X * 64f + 32f, tile.Y * 64f + 64f - 4f + yDrawOffset)), Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 4f, SpriteEffects.None, 1E-06f);
			}
		}
		spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(tile.X * 64f + (float)((getEffectiveSize() + 1) * 64 / 2), (tile.Y + 1f) * 64f - (float)((getEffectiveSize() > 0 && (!townBush.Value || getEffectiveSize() != 1) && size.Value != 4) ? 64 : 0) + yDrawOffset)), sourceRect.Value, Color.White, shakeRotation, new Vector2((getEffectiveSize() + 1) * 16 / 2, 32f), 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)(getBoundingBox().Center.Y + 48) / 10000f - tile.X / 1000000f);
	}
}
