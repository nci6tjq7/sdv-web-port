...
public class Mummy : Monster
{
	...
	public override int takeDamage(int damage, int xTrajectory, int yTrajectory, bool isBomb, double addedPrecision, Farmer who)
	{
		...
		else
		{
			...
			if (base.Health <= 0)
			{
				if (!isBomb && who.CurrentTool is MeleeWeapon meleeWeapon && meleeWeapon.hasEnchantmentOfType<CrusaderEnchantment>())
				{
					Utility.makeTemporarySpriteJuicier(new TemporaryAnimatedSprite(44, base.Position, Color.BlueViolet, 10)
					{
						holdLastFrame = true,
	...
	private List<FarmerSprite.AnimationFrame> getCrumbleAnimation(bool reverse = false)
	{
		...
		{
			list.Add(new FarmerSprite.AnimationFrame(16, 100, 0, secondaryArm: false, flip: false, behaviorAfterRevival, behaviorAtEndOfFrame: true));
		}
		...
		if (!reverse)
		{
			list.Add(new FarmerSprite.AnimationFrame(19, 100, 0, secondaryArm: false, flip: false, behaviorAfterCrumble));
		}
		else
		{
