...
public class BigSlime : Monster
{
	...
	protected override void updateAnimation(GameTime time)
	{
		...
		if (Utility.isOnScreen(base.Position, 128) && Sprite.currentFrame == 0 && currentFrame == 7)
		{
	...
	public override void draw(SpriteBatch b)
	{
		if (!base.IsInvisible && Utility.isOnScreen(base.Position, 128))
		{
			...
			heldItem.Value?.drawInMenu(b, getLocalPosition(Game1.viewport) + new Vector2(28f, -16f + (float)Math.Sin(heldObjectBobTimer + 1f) * 4f), 1f, 1f, (float)(y - 1) / 10000f, StackDrawType.Hide, Color.White, drawShadow: false);
			b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(56f, 16 + yJumpOffset), Sprite.SourceRect, c.Value, rotation, new Vector2(16f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f)));
			if (isGlowing)
			{
