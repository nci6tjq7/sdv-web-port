	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		if (pupating.Value)
		{
			...
			if (metamorphCounter <= 0)
			{
				...
				if (base.currentLocation is MineShaft mineShaft)
				{
					base.currentLocation.characters.Add(mineShaft.BuffMonsterIfNecessary(new Fly(base.Position, hard.Value)
					{
		...
		else if (base.Health <= base.MaxHealth / 2 - 2 || (hard.Value && base.Health < base.MaxHealth))
		{
			...
			Point standingPixel3 = base.Player.StandingPixel;
			if (Math.Abs(standingPixel3.Y - standingPixel2.Y) > 128)
			{
				if (standingPixel3.X > standingPixel2.X)
				{
			...
			else if (Math.Abs(standingPixel3.X - standingPixel2.X) > 128)
			{
				if (standingPixel3.Y > standingPixel2.Y)
				{
					SetMovingUp(b: true);
				}
				else
