	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		else if (throwing.Value)
		{
			...
			if (Sprite.Animate(time, 20, 4, 150f))
			{
				...
				faceDirection(2);
				Vector2 velocityTowardPlayer = Utility.getVelocityTowardPlayer(new Point((int)base.Position.X, (int)base.Position.Y), 8f, base.Player);
				if (isMage.Value)
				{
					if (Game1.random.NextBool())
					{
						base.currentLocation.projectiles.Add(new DebuffingProjectile("19", 14, 4, 4, (float)Math.PI / 16f, velocityTowardPlayer.X, velocityTowardPlayer.Y, new Vector2(base.Position.X, base.Position.Y), base.currentLocation, this));
					}
					else
					{
						base.currentLocation.projectiles.Add(new BasicProjectile(base.DamageToFarmer * 2, 9, 0, 4, 0f, velocityTowardPlayer.X, velocityTowardPlayer.Y, new Vector2(base.Position.X, base.Position.Y), "flameSpellHit", "flameSpell", null, explode: false, damagesMonsters: false, base.currentLocation, this));
					}
				}
				else
				{
					base.currentLocation.projectiles.Add(new BasicProjectile(base.DamageToFarmer, 4, 0, 0, (float)Math.PI / 16f, velocityTowardPlayer.X, velocityTowardPlayer.Y, new Vector2(base.Position.X, base.Position.Y), "skeletonHit", "skeletonStep", null, explode: false, damagesMonsters: false, base.currentLocation, this));
				}
			}
		}
