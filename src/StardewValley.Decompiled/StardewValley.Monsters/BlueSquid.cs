...
public class BlueSquid : Monster
{
	...
	public override Rectangle GetBoundingBox()
	{
		...
		return new Rectangle((int)vector.X, (int)vector.Y + 16, width, 64);
	}

	public override void drawAboveAllLayers(SpriteBatch b)
	{
		int y = base.StandingPixel.Y;
		b.Draw(Game1.shadowTexture, getLocalPosition(Game1.viewport) + new Vector2(32f, 96f), Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), Math.Min(4f, 4f + (float)squidYOffset / 20f), SpriteEffects.None, (float)(y - 32) / 10000f);
		b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, 21 + squidYOffset) + new Vector2((shakeTimer > 0) ? Game1.random.Next(-2, 3) : 0, (shakeTimer > 0) ? Game1.random.Next(-2, 3) : 0), Sprite.SourceRect, Color.White, lastRotation.Value, new Vector2(12f, 12f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)y / 10000f)));
	}
	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		if (projectileIntroTimer.Value <= 0f && projectileOutroTimer.Value <= 0f)
		{
			...
			{
				Vector2 velocityTowardPoint = Utility.getVelocityTowardPoint(findPlayer().position.Value, position.Value, Game1.random.Next(25, 50));
				...
				{
					justThrust = true;
					Vector2 velocityTowardPoint2 = Utility.getVelocityTowardPoint(findPlayer().position.Value, position.Value + new Vector2(Game1.random.Next(-64, 64)), Game1.random.Next(25, 50));
					velocityTowardPoint2.X *= -1f;
					setTrajectory(velocityTowardPoint2);
					lastRotation.Value = (float)Math.Atan2(0f - yVelocity, xVelocity) + (float)Math.PI / 2f;
		...
		if (projectileIntroTimer.Value > 0f)
		{
			...
			if (projectileIntroTimer.Value < 0f)
			{
				...
				Halt();
				Point standingPixel = base.StandingPixel;
				Vector2 velocityTowardPlayer = Utility.getVelocityTowardPlayer(standingPixel, 8f, base.Player);
				DebuffingProjectile debuffingProjectile = new DebuffingProjectile("27", 8, 3, 4, 0f, velocityTowardPlayer.X, velocityTowardPlayer.Y, Utility.PointToVector2(standingPixel) - new Vector2(32f, -squidYOffset), base.currentLocation, this);
				debuffingProjectile.height.Value = 48f;
				base.currentLocation.projectiles.Add(debuffingProjectile);
