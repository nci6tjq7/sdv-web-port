...
public class PlayerCoupleBirthingEvent : BaseFarmEvent
{
	...
	private FarmHouse chooseHome()
	{
		...
		list.Sort((Farmer p1, Farmer p2) => p1.UniqueMultiplayerID.CompareTo(p2.UniqueMultiplayerID));
		foreach (Farmer item in list)
		{
			if (Game1.getLocationFromName(item.homeLocation.Value) is FarmHouse farmHouse && farmHouse == item.currentLocation && isSuitableHome(farmHouse))
			{
				return farmHouse;
			}
		}
		foreach (Farmer item2 in list)
		{
			if (Game1.getLocationFromName(item2.homeLocation.Value) is FarmHouse farmHouse2 && isSuitableHome(farmHouse2))
			{
				return farmHouse2;
			}
		}
	...
	public override bool setUp()
	{
		...
		isPlayersTurn = spouseFriendship.Proposer != Game1.player.UniqueMultiplayerID == (farmHouse.getChildrenCount() % 2 == 0);
	...
	public void afterMessage()
	{
		if (isPlayersTurn)
		{
			...
			bool isDarkSkinned = Utility.CreateRandom(Game1.uniqueIDForThisGame, Game1.stats.DaysPlayed).NextDouble() < num;
