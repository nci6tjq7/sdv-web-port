...
public class SaveMigrator_1_4 : ISaveMigrator
{
	...
	public bool ApplySaveFix(SaveFixes saveFix)
	{
		...
		case SaveFixes.StoredBigCraftablesStackFix:
			...
			{
				if (item is Object @object && @object.bigCraftable.Value && @object.Stack == 0)
				{
					@object.Stack = 1;
				}
				return true;
			...
			Utility.ForEachLocation(delegate(GameLocation location)
			{
				if (location is DecoratableLocation)
				{
					...
					for (int l = 0; l < location.furniture.Count; l++)
					{
						...
						{
							StorageFurniture item2 = new StorageFurniture(furniture.ItemId, furniture.TileLocation, furniture.currentRotation.Value);
							list.Add(item2);
							location.furniture.RemoveAt(l);
							l--;
						}
					}
					foreach (Furniture item3 in list)
					{
						location.furniture.Add(item3);
					}
				}
				return true;
			});
		...
		case SaveFixes.RevealSecretNoteItemTastes:
		{
			...
			for (int i = 0; i < 21; i++)
			{
				if (dictionary.TryGetValue(i, out var value) && Game1.player.secretNotesSeen.Contains(i))
				{
