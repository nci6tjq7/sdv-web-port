...
public class Woodpecker : Critter
{
	...
	public override bool update(GameTime time, GameLocation environment)
	{
		...
		if (!flyingAway && peckTimer <= 0 && sprite.CurrentAnimation == null)
		{
			...
			for (int i = 0; i < num; i++)
			{
				list.Add(new FarmerSprite.AnimationFrame(baseFrame, 100));
				list.Add(new FarmerSprite.AnimationFrame(baseFrame + 1, 100, secondaryArm: false, flip: false, playPeck));
			}
			list.Add(new FarmerSprite.AnimationFrame(baseFrame, 200, secondaryArm: false, flip: false, donePecking));
		...
		{
			Character character = Utility.isThereAFarmerOrCharacterWithinDistance(position / 64f, 6, environment);
			characterCheckTimer = 200;
			if ((character != null || tree.stump.Value) && !flyingAway)
			{
				flyingAway = true;
				if (character != null && character.Position.X > position.X)
				{
					flip = false;
				}
				else
