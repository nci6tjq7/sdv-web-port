...
```mermaid
graph TD
    NTSI{Need to show information?}
    DWIMG{Decision where I might go wrong?}
    UM[Use markdown]
    SIF[Small inline flowchart]
...
Flowcharts use Mermaid syntax (rendered natively by most platforms). See
`graphviz-conventions.dot` in this directory for upstream's Graphviz style
rules (used by `render-graphs.js` on Claude Code; z.ai renders Mermaid directly).
