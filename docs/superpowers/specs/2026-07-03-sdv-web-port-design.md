...
// 两条实现
class FileSystemAccessApiVfs : IVirtualFileSystem { ... }  // A2
class OpfsVfs : IVirtualFileSystem { ... }                  // A1
```
...
| 项 | 值 | 说明 |
|---|---|---|
| WASM 内存上限 | 4GB | 通过 `--max-memory=4GB` 启用 |
| 实际占用预期 | 1.5-2.5GB | SDV 1.2GB + 运行时 + KNI 开销 |
| 启用 WASM Threads | 是 | Silk.NET 需 threading 支持 |
| 启用 SIMD | 视浏览器 | 自动降级 |
...
<PropertyGroup>
  ...
  <WasmRuntimeExecutionMode>InterpreterAndAOT</WasmRuntimeExecutionMode>
  <WasmEnableJiterpreter>true</WasmEnableJiterpreter>
  <WasmMainJSPath>main.js</WasmMainJSPath>
  <EmccCompileOnly>false</EmccCompileOnly>
</PropertyGroup>
```
...
**KNI 的 `NativeGL` 后端 + Silk.NET.OpenGL.WASM**：
- Silk.NET 直接绑定 WebGL2 上下文
- KNI 通过 Silk.NET 调用 GL API
- 避免 BlazorGL 中间层
...
Phase 3b (2 周): Harmony → RuntimeDetour 兼容 shim
...
- Android 移植已验证此路线
- WASM 上无 JIT，RuntimeDetour 通过 IL 重写 + interpreter 跳转实现
- 风险：WASM 沙箱下 IL Emit 行为未完全验证
...
**目标**：浏览器黑屏 + 1 帧 MonoGame Clear 调用成功

**验收**：
- [ ] dotnet 10 + Uno.Wasm.Bootstrap 项目可构建
- [ ] 浏览器加载 WASM 包不报错
- [ ] Canvas 显示一帧指定颜色
- [ ] KNI NativeGL 后端初始化成功
...
| R4 | 内存超 2GB 在低端设备崩溃 | 15% | 移动端不可用 | 桌面端优先，Phase 6+ 再考虑移动 | OOM 报告 |
